...
public class StudentFram extends JFrame {
	...
	public StudentFram(Dormitory dor) {
		 try {
			 ...
			 * 寮虹珛浣撳崐閫忔槑	translucencyAppleLike
			 * 寮辩珛浣撴劅鍗婇�忔槑	translucencySmallShadow
			 * 鏅�氫笉閫忔槑	 generalNoTranslucencyShadow
			 * 绯荤粺榛樿鏍峰紡	osLookAndFeelDecorated
			 */
...
//		setBounds(100, 100, 450, 300);
	...
	public void getDuctTable(){
		String sql="SELECT time,reason,points FROM  deduction_record where dorId='"+dor.getName().replaceAll(" ", "")+"' ORDER BY points DESC" ;
//		String sql="SELECT dorId, address, points FROM Dormitory ORDER BY points DESC";
		query.Query(sql);
